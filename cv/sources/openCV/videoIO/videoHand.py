# 손을 검출(살색을 검출)하여 손가락의 갯수(두 선이 연결된 지점)를 카운트 하는 방법
import cv2 as cv
import numpy as np
import os


# (detect) 특정 공간 만큼 얼굴을 삭제(검게 표시) haarcascade_frontalface_alt.xml을 사용함 / 얼굴을 검출하고,
...
# (remove) 특정 공간 만큼 얼굴을 삭제(검게 표시) haarcascade_frontalface_alt.xml을 사용함 / 검출된 얼굴의 일정 영역을 검정칠
...
# H, S의 범위를 정하여 살색을 검출하여 바이너리 이미지를 도출함 (실험환경에 맞춰함, 수정가능)
...
# 컨투어 영역 컨펙설(초록색)에서 손가락을 찾음(두 선이 만나는 지점) // 손가락을 굽혀도 문제가 됨(제거해야 함)
def getFingerPosition(max_contour, img_result, debug):
    ...
    # STEP 6-2
    # 두 개의 손가락 사이에 위치한 디펙서(손가락과 손가락 사이 들어가는 부분) // 뾰족한 부분을 검출함
    ...
    # STEP 6-3
    # 컴펙설(convexhull)을 통해 검출한 손가락 후보(초록색)과 디펙서를 통해 검출한 손가락 후보를 합쳐서 사용함 // 제거해야할 손가락 후보가 있음
    points = points1 + points2
    points = list(set(points))

    # STEP 6-4
    # 손가락 후보가 발견된 곳은 2개의 edge가 만나는 점이고, 이점을 이용해서 좌우의 엣지가 90도 이하 인 것만 손가락으로 인식
    new_points = []
    for p0 in points:
...
def process(img_bgr, debug):
    ...
    # STEP 3
    # 잔상(5*5 크기)을 없애주기 위해서 모폴로지 연산을 수행함 // 테스트 환경에 따라 변경
    kernel = cv.getStructuringElement(cv.MORPH_ELLIPSE, (5, 5))
    img_binary = cv.morphologyEx(img_binary, cv.MORPH_CLOSE, kernel, 1)
    cv.imshow("Binary", img_binary)

    # STEP 4
    # 바이너리 이미지에서 컨투어를 검출하는 부분 (흰색영역의 외곽을 따라 컨투어를 도출함)
