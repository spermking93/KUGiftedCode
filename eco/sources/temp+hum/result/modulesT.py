...
def LinearRegression(step, lr):
    ...
    # y1값 데이터에 위에서 측정된 온도리스트를 저장합니다. 
    y_data = templist
      
    # 구하고자 하는 온도의 일차함수의 y절편의 값을 i=0(t=0)에서의 온도값으로 정합니다.
    b_initial = templist[0]
    
    # 구하고자 하는 온도의 일차함수의 기울기를 i=1(t=0.01s)와 i=8(t=0.08s)일때의 온도값 데이터로 구합니다. 
    W_initial = (templist[8] - templist[1]) / 8.
